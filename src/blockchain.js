...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        return new Promise(async (resolve, reject) => {
            ...
            const differenceInMinutes = this.diff_minutes(time, currentTime);
            if(differenceInMinutes <= 5)
            {
                ...
                reject('Stars must be declared before 5 minutes after signature')
            }
        });
    }

    diff_minutes = (dt2, dt1) => {
        //sense we trim the last 3 digits we are already in seconds so we just need to convert to minutes
        return Math.abs(dt1 - dt2) /60;   
     }
    ...
    getBlockByHash(hash) {
        return new Promise((resolve, reject) => {
            const foundBlock = this.chain.filter(block => block.hash === hash)
            return foundBlock.length > 0 ? resolve(foundBlock) : resolve('Hash not found');
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            try{
                ...
                stars.length > 0 ? resolve(stars) : resolve("No stars associated with Address");
            } catch(msg){
                reject(msg);
            }
        });
